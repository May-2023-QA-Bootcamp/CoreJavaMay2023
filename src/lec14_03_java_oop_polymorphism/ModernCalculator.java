...
public class ModernCalculator extends LandCalculator {
	...
	public int landCalculator(int a, int b, int c) { // Parameter
		int total1 = a/2 + b + c+6; // value of the local variable changed
	...
	*/
	...
	public void landCalculator(int a, String b, int c, int d, int e, int f) {
		...
		System.out.println("Total area from Modern calculator: " + total8);
	}
	
	@Override
	public void landCalculator(int a, int c, String b,  int d, int e, int f) {
		int total9 = a + c+  Integer.parseInt(b) + d/3 + e + f;
		System.out.println("Total area from Modern calculator: " + total9);
	}
